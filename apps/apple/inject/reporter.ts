/**
 * 注入 left/right webview 的上报+执行脚本(content.ts 的 WKWebView 平移)。
 * 由 Swift 以 WKUserScript(documentStart)在每个文档加载前注入,并先注入
 * window.__DC_VIEW__ = "w1-left"|"w1-right" 标明所在视图。
 *
 * 上行:window.webkit.messageHandlers.dcReport.postMessage(JSON 字符串)
 * (字符串载荷零类型转换坑,与 CDP binding 同款先例)。
...
(() => {
    ...
    try {
      (
        window as unknown as {
          webkit?: { messageHandlers?: { dcReport?: { postMessage: (s: string) => void } } };
        }
      ).webkit?.messageHandlers?.dcReport?.postMessage(JSON.stringify({ view, ...msg }));
    } catch (e) {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      // 允许任意绝对 scheme(含自测用的 dcapp://),只排除脚本/邮件类
  ...
  // SPA history 路由(user script 注入主世界时可拦截;失败则由 hashchange 兜底)
  try {
  ...
  // ---- 缩放快捷键(Catalyst/外接键盘):Ctrl/Cmd + =/-/0 → 原生页面缩放
  //  (两侧同步)。键盘焦点通常在某个内容页里,所以监听放在 reporter;
  //  controller 页自己也有一份(焦点在工具条时)。 ----
  ...
  let lastReportAt = 0;
  /**
   * 程序滚动(我们 apply 引起)期间不上报,直到滚动停稳(连续两帧位置不变)
  ...
  // user script 在 document_start 注入,此时 body 尚未出现,观察 documentElement。
  // WKWebView 的 document_start 时 documentElement 也可能未就绪(为 null),
  ...
  /**
   * iOS WKWebView 的顺滑滚动对远距离目标会中途弃程(约一屏即停,
   * WebKit 已知行为;桌面无此问题)。先 smooth 保近距动画,700ms 后
   * 校验落点,偏了用瞬时滚动补正。
   */
  function ensureLanded(check: () => boolean, fix: () => void): void {
    setTimeout(() => {
      if (check()) return;
      fix();
      markProgrammatic();
  ...
  function scrollToAnchor(anchor: string): void {
    ...
    const attempt = (): void => {
      ...
      if (el) {
        ...
        ensureLanded(
          () => Math.abs(el!.getBoundingClientRect().top - 64) <= 80,
  ...
  (window as unknown as { __dcApply: (msg: unknown) => void }).__dcApply = (msg: unknown) => {
    ...
    switch (m?.t) {
      ...
      case 'bg:scroll': {
        ...
        ensureLanded(
          () => Math.abs(window.scrollY - top) <= 80,
